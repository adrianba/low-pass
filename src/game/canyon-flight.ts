...
import { MAX_TRACK_COMPONENT, MAX_TRACK_KNOTS } from '../simulation/flight-track-data';
...
function buildFlight(count: number, previous: Pose, releaseZ: number, offset: number, side: number, phase: number, target: Vec3): CanyonFlight {
  ...
  const track = new FlightTrack(path, u => Math.min(d.speed,
    Math.sqrt(70 / Math.max(0.000001, Math.abs(routeMotion(releaseZ + d.speed * u).curvature)))), start, TRACK_END_PHASE);
...
export type CanyonCandidate = { ok: true; encounter: Encounter } | { ok: false; reason: string };

function validateCanyonStart(count: number, previous: Pose): void {
  if (!Number.isSafeInteger(count) || count < 0 ||
    ![previous.position, previous.velocity, previous.acceleration].every(v =>
      [v.x, v.y, v.z].every(n => Number.isFinite(n) && Math.abs(n) <= MAX_TRACK_COMPONENT)) ||
    ![previous.bank, previous.pitch].every(n => Number.isFinite(n) && Math.abs(n) <= Math.PI)) {
    throw new Error('Invalid canyon shelf candidate start.');
  }
}

export function canyonShelves(count: number, previous: Pose): number[] {
  validateCanyonStart(count, previous);
  ...
  return Array.from({ length: 12 }, (_, offset) => first + offset)
    .filter(index => index % 2 === 0 || d.speed <= 170);
}

export function canyonCandidate(count: number, seed: number, previous: Pose, index: number, phaseOffset = 0): CanyonCandidate {
  validateCanyonStart(count, previous);
  if (!Number.isFinite(seed) || !Number.isSafeInteger(index) ||
    Math.abs(CANYON.shelfOrigin + index * CANYON.shelfSpacing) > MAX_TRACK_COMPONENT ||
    !Number.isFinite(phaseOffset) || Math.abs(phaseOffset) > Math.PI ||
    !Number.isFinite(CANYON.shelfOrigin + index * CANYON.shelfSpacing)) throw new Error('Invalid canyon shelf candidate inputs.');
  const d = difficulty(count), fall = Math.sqrt(2 * (ATTACK_HEIGHT - 2.2) / GRAVITY);
  ...
  const phase = -0.65 - hash(count, 491, seed) * 0.15 + phaseOffset;
  ...
  if (Math.abs(offset) > 115) return { ok: false, reason: 'offset' };
  const start = (projectRoute(previous.position.x, previous.position.z).along - releaseZ) / d.speed;
  if (start >= 0 || Math.floor(TRACK_END_PHASE * 10) - Math.ceil(start * 10) + 2 > MAX_TRACK_KNOTS) {
    return { ok: false, reason: 'coverage' };
  }
  const flight = buildFlight(count, previous, releaseZ, offset, side, phase, target);
  if (flight.entryEnd >= 0) return { ok: false, reason: 'entry-coverage' };
  ...
  if (impact.kind !== 'ground' || distance(impact, target) > 0.05) return { ok: false, reason: 'impact' };
  if (speedOf(flight.track.at(0)) < d.speed * 0.85) return { ok: false, reason: 'attack-speed' };
  if (!flight.track.respectsSpeed(d.speed)) return { ok: false, reason: 'track-speed' };
  ...
  if (!entryValid) return { ok: false, reason: 'entry-speed' };
  ...
  if (!clear) return { ok: false, reason: 'clearance/speed' };
  ...
  if (!visible) return { ok: false, reason: `visibility ${visibilityFailure}` };
...
export function planCanyon(count: number, seed: number, previous: Pose): Encounter {
  ...
  for (const index of canyonShelves(count, previous)) {
    const candidate = canyonCandidate(count, seed, previous, index);
    if (candidate.ok) return candidate.encounter;

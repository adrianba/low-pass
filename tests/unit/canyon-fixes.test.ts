...
import { difficulty, TARGET_RADIUS, STEP } from '../../src/config/game';
import { canyonSightDistance } from '../../src/game/canyon-flight';
  ...
  it('sees all target rings before the dive deadline on sequential courses', () => {
    ...
    for (let seed = 0; seed < 8; seed++) {
      ...
      for (let count = 0; count < 40; count++) {
        ...
        const deadline = -difficulty(count).diveDuration - 0.5;
        ...
        if (!screen || screen.x < 0.05 || screen.x > 0.95 || screen.y < 0.1 || screen.y > 0.94
          || range > canyonSightDistance(count) || occluded) failures.push({ seed, pass: count + 1,
          target: encounter.target, range, allowed: canyonSightDistance(count), screen, occluded });
      ...
      for (const aspect of [0.75, 16 / 9, 32 / 9]) {
        ...
        for (let count = 0; count < 24; count++) {
          ...
          while (run.encounter.time <= deadline) {
            ...
            if (targetInChaseView(run.encounter.target, view, run.surface, aspect, canyonSightDistance(count))) {
              run.seeTarget();
              break;
            }

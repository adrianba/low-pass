...
async function tick() {
  ...
  try {
    ...
    if (drops < 2 && match.frame.time >= flight.attempts[slot].releaseAt && match.frame.ready && match.release()) drops++;
  } finally { busy = false; }
}
const fixture = {
  async connect(member: RoomMembership, terrain: TerrainTheme) {
    lobby = await LobbyConnection.connect(member, { ...DEFAULT_SETTINGS, terrain, assist: false }, 'direct', 7, versions, prepared => {
      try {
        match = new MatchController(prepared, (_slot, view, range) => ({ ...view, range, aspect: 1.15 }));
        match.assetsLoaded();
      } catch (error) {

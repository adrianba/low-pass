...
describe('real private room HTTP boundary', () => {
  it('authorizes, atomically reserves a guest, admits, and revokes both capabilities on leave', async () => {
    ...
    expect(await (await fetch(origin + '/api/multiplayer/capabilities')).json()).toEqual({ multiplayer: false, reason: 'not_implemented', rooms: true, signaling: true, turn: false });
